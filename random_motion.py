...
class Motion(IntensityTransform, FourierTransform):
    ...
    def get_rigid_transforms(
        ...
        identity = np.eye(4)
        matrices = [identity]
        #! copied from the affine transform within the torchio library
        #! =============================================================================
        ...
        for degrees, translation in zip(degrees_params, translation_params):
            #// the last dimention seems to be wrong, so I need to flip the sign for the z-rotation and z-translation parameter.
            #// Not sure why yet, so this is just a temporary fix.
            #! A correct fix is now applied, hence deleted the above comment
            radians = np.radians(degrees).tolist()
            # #! =============================================================================
            #! apply the ras_to_lps function to the degrees and translation, SimpleITK uses LPS
            radians = ras_to_lps(radians)
            translation = ras_to_lps(translation)
            # #! =============================================================================
            ...
            #! The original comments from the random_affine.py file explain the following:
            ...
            #! therefore modification is made to fix the issue
            #! Now it is the same as the affine transform within the torchio library
            motion = motion.GetInverse()
            #? =====================original code=====================
            # motion_matrix = self.transform_to_matrix(motion)
            # matrices.append(motion_matrix)
            #? =======================================================
            #! changing for testing, with this it is applying rotation w.r.t the center of the image
            matrices.append(motion)
    ...
    def add_artifact(
        ...
        for image in images:
            ...
            # #! add one to the testing index
            # test_idx += 1
        #! Comment out the following line to disable the sorting of the spectra
        ...
        for spectrum, fin in zip(spectra, indices):
            ...
            #* =============================================================================
            # in order to check the idx, I add one print comment here
            # print("ini: ", ini, "fin: ", fin)
            # this uses 'AP' as the second phase encoding direction (this is added by me for experimentation purposes)
            result_spectrum[:, ini:fin,:] = spectrum[:, ini:fin,:]
            #* =============================================================================

            # this uses 'IS' as the second phase encoding direction (this is the original code in the torchio library)
            # result_spectrum[..., ini:fin] = spectrum[..., ini:fin]
            ini = fin
            
        #? =====================original code=====================
        # original code output the real part of the inverse fourier transform only which is not correct
        # result_image = self.inv_fourier_transform(result_spectrum).real.float()
        # ? =======================================================
            
        # the correct way is using absolute value but here I choose to output the complex valued image for experimentation purposes
        result_image = torch.abs(self.inv_fourier_transform(result_spectrum)).float()
